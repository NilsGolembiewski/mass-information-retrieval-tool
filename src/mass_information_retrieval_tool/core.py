...
from googleapiclient.discovery import build as build_google_service # Renamed to avoid conflict
...
def load_config(config_path: str) -> Dict[str, Any]:
    ...
    required_keys = [
        'input_csv', 'output_csv', 'key_columns', 'gemini_model', 'api_key_env_var',
        'schema', 'query_generation_prompt_template', 'final_processing_prompt_template',
        'google_search_api_key_env_var', 'google_cse_id_env_var'
    ]
    missing_keys = [key for key in required_keys if key not in config]
    if missing_keys:
        raise ValueError(f"Missing required keys in configuration: {', '.join(missing_keys)}")
    ...
    elif not isinstance(config['google_search_num_results'], int) or config['google_search_num_results'] <= 0:
        raise ValueError("Configuration 'google_search_num_results' must be a positive integer.")

    # Validate prompt templates are strings
    if not isinstance(config['query_generation_prompt_template'], str):
        raise ValueError("Configuration 'query_generation_prompt_template' must be a string.")
    if not isinstance(config['final_processing_prompt_template'], str):
        raise ValueError("Configuration 'final_processing_prompt_template' must be a string.")
...
def configure_google_search(api_key_env_var: str, cse_id_env_var: str) -> Optional[Any]:
    ...
    if not api_key:
        logging.error(f"Google Search API key environment variable '{api_key_env_var}' not found.")
        return None # Indicate failure
    if not cse_id:
        logging.error(f"Google Search CSE ID environment variable '{cse_id_env_var}' not found.")
        return None # Indicate failure
    ...
    try:
        ...
        logging.error(f"Failed to build Google Custom Search service: {e}", exc_info=True)
        return None
...
def call_gemini_for_query(client: genai.Client, model_name: str, prompt: str) -> Optional[str]:
    ...
    try:
        ...
        logging.exception(f"Gemini API call for query generation failed (prompt: {prompt[:100]}...):")
        return None

# --- New Function: Execute Google Search ---
def execute_google_search(service: Any, cse_id: str, query: str, num_results: int) -> Optional[str]:
    ...
    try:
        ...
        logging.error(f"Google Search API HTTP error for query '{query}': {e}", exc_info=True)
        return None
    except Exception as e:
        logging.error(f"Unexpected error during Google Search for query '{query}': {e}", exc_info=True)
        return None
...
def execute_all_google_searches(
    ...
    for field_name, query in queries.items():
        if query: # Only search if a query was successfully generated
            result_text = execute_google_search(search_service, cse_id, query, num_results)
            search_results[field_name] = result_text
        else:
            search_results[field_name] = None # Mark as None if query generation failed
            logging.warning(f"Skipping search for field '{field_name}' due to missing query.")
...
def generate_search_queries_for_row(
    ...
    for field_name in schema.keys():
        prompt = query_template.format(key_values_json=key_values_json_str, field_name=field_name)
...
def call_gemini_for_processing(
    ...
    try:
        ...
        logging.warning(f"Failed to decode final JSON response: {e}\nResponse text: {response.text[:500]}...")
        return None
    except Exception as e:
...
def process_row_multi_step(
    ...
    """
    key_values_json_str = json.dumps(key_values)
    logging.info(f"--- Processing row {index + 1}: Key Values = {key_values_json_str} ---")
    ...
    search_service = configure_google_search(
        config['google_search_api_key_env_var'],
        config['google_cse_id_env_var']
    )
    if not search_service:
        logging.error(f"Row {index + 1}: Failed to configure Google Search service in thread. Skipping searches.")
    ...
    for field_name, result_text in search_results.items():
        ...
        if result_text:
            aggregate_results_text += f"Results for '{field_name}' (query: '{query}'):\n{result_text}\n\n"
        else:
            aggregate_results_text += f"No results found for '{field_name}' (query: '{query}').\n\n"
...
def process_csv(config: Dict[str, Any]) -> None:
    ...
    for index, row in df.iterrows():
        ...
        if len(cleaned_key_values) < len(key_values):
             logging.warning(f"Row {index + 1} has missing value(s) in key columns: {key_values}. Using available keys: {cleaned_key_values}")
        if not cleaned_key_values:
            logging.error(f"Row {index + 1} has NO valid key values. Skipping processing for this row.")
    ...
    results = Parallel(n_jobs=max_parallel_requests, backend="threading")(
        ...
        ) for index, key_vals in job_inputs if key_vals is not None # Skip rows marked for skipping
    )
    ...
    logging.info(f"Processing finished. Successfully retrieved data for {update_count} rows out of {total_processed} processed rows ({total_skipped} rows skipped due to missing keys).")
